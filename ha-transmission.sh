...
#set -x; set -o functrace  # in case of debugging, break glass (uncomment)
...
echo "$DATE: Starting torrent container" > "$ACTIVITY_LOG"
echo "$DATE: Starting torrent container" > "$OPENVPN_LOG"
echo "$DATE: Starting torrent container" > "$TRANS_LOG"
echo "$DATE: Starting VPN..." >> "$ACTIVITY_LOG"
...
until [[ $(ip tuntap 2> /dev/null | wc -l) > 0 ]]; do
    echo "$DATE: VPN is connecting..." >> "$ACTIVITY_LOG"
    sleep "$SLEEP_SHORT"
done
...
# Start transmission
transmission-daemon --logfile "$TRANS_LOG" &
...
DEFAULT_ROUTE=$(ip route show dev tun0 | awk '{if($1 ~ /.*0.0.0.*/) {print $3}}' | uniq)
...
while true; do
    ...
    if [[ "$?" == 0 ]]; then
        ...
        echo "$DATE: Check succeeded." >> "$ACTIVITY_LOG"
        sleep "$SLEEP_LONG"
        continue
    fi
    ...
    # log failure
    echo "$DATE: Check failed, restarting VPN..." >> "$ACTIVITY_LOG"
    ...
    until [[ $(ip tuntap 2> /dev/null | wc -l) == 0 ]]; do
        echo "$DATE: Waiting on VPN teardown..." >> "$ACTIVITY_LOG"
        sleep "$SLEEP_SHORT"
    done
    ...
    #until [[ $(ip tuntap | wc -l) > 0 ]]; do
    until [[ $(ip tuntap 2> /dev/null | wc -l) > 0 ]]; do
        echo "$DATE: Waiting on VPN connection..." >> "$ACTIVITY_LOG"
        sleep "$SLEEP_SHORT"
    done

    # Log the vpn up success
    echo "$DATE: VPN is back up!" >> "$ACTIVITY_LOG"
